...
def value_bet(id, df_matches, coeff, best_odd, limit=40):
    ...
    if rating in range(-limit, limit+1):
        ...
        a = list(coeff['AwayPoly'])[0]*rating + list(coeff['AwayPoly'])[1]
        # d = 1 - (a+h)
        # h = 1/h
        a = 1/a
        # d = 1/d
        ...
        # if best_odd > h + 0.5:
        #     return 'H'
        if df_matches.at[id, 'B365A'] >= a*1.02:
             return 'A'
...
if __name__ == '__main__':
    ...
    for s in '','b':
        for y in year:
            ...
            df_matches = df_matches[['HomeTeam', 'AwayTeam', 'FTHG', 'FTAG', 'FTR', 'B365A', 'WHA', 'BWA', 'PSA', 'IWA', 'VCA']]
            ...
            for id in eligible:
                best_odd = max(df_matches.iloc[id][['B365A', 'WHA', 'BWA', 'PSA', 'IWA', 'VCA']])
            ...
            with open(f'./final_test_away/test_{y}{s}.json', 'w') as fp:
                dump(results, fp)
            if s == 'a':
                s = ''
